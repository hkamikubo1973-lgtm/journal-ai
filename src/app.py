...
from receivable_legacy_template_service import (
    build_empty_epson_template_row, normalize_receivable_template_text,
    is_receivable_text_close, is_receivable_account_name, is_cash_account_name,
    get_receivable_template_quality, parse_receivable_template_date,
    is_receivable_sub_template_match, is_receivable_summary_template_match,
    is_receivable_template_text_match,
    find_receivable_template_match as _find_receivable_template_match,
    find_receivable_template_row as _find_receivable_template_row,
)


def find_receivable_template_match(journal, customer_name="", source_candidates=None):
    return _find_receivable_template_match(journal, customer_name, source_candidates, records)


def find_receivable_template_row(journal, source_candidates, customer_name):
    return _find_receivable_template_row(journal, source_candidates, customer_name, records)
...
from past_journal_import_service import (
    read_past_journal_csv, load_transactions_df, prepare_past_journal_import,
    append_past_journals_to_transactions as _append_past_journals_to_transactions,
)

def append_past_journals_to_transactions(new_df):
    count = _append_past_journals_to_transactions(new_df)
    if count:
        st.cache_data.clear()
    return count
